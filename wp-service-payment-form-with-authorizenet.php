...
function wpspf_plugin_settings_page() {
	if(isset($_POST['submit'])){
		$wpspfnet_enable = trim($_POST['wpspfnet_enable']);
		$wpspf_apiloginid = trim($_POST['wpspf_apiloginid']);
		$wpspf_transactionkey = trim($_POST['wpspf_transactionkey']);
		$wpspf_transactionmode = trim($_POST['wpspf_transactionmode']);
		$wpspf_paymentheading = trim($_POST['wpspf_paymentheading']);
		$wpspf_servicetype = trim($_POST['wpspf_servicetype']);
		
		$wpspfnet_enable_servicetype = trim($_POST['wpspfnet_enable_servicetype']);
...
?>
<div class="wrap">
<h3><?php echo __( 'WP Service Payment Form With Authorize.net Plugin For Wordpress', 'wpspf_with_authorize.net' ); ?></h3>
<p><?php echo __( 'Please use "[wpspf-paymentform]" shortcode for payment form.', 'wpspf_with_authorize.net' ); ?></p>
<form method="post" action="">
    <table class="form-table">
		
		<tr valign="top">
        <th scope="row"><?php echo __( 'Payment Heading', 'wpspf_with_authorize.net' ); ?></th>
        <td><input type="text" style="width:100%;" name="wpspf_paymentheading" value="<?php echo get_option( 'wpspf_paymentheading' ); ?>" required="required" /></td>
        </tr>
		
		<tr valign="top">
        <th scope="row"><?php echo __( 'Enable/Disable', 'wpspf_with_authorize.net' ); ?></th>
        <td><input type="checkbox" name="wpspfnet_enable_servicetype" value="1" <?php if ( trim(get_option( 'wpspfnet_enable_servicetype' ))==1 ){ echo 'checked'; } ?> /><?php echo __( 'Check to show service type on front end', 'wpspf_with_authorize.net' ); ?></td>
        </tr>
		
		<tr valign="top">
        <th scope="row"><?php echo __( 'Service type', 'wpspf_with_authorize.net' ); ?></th>
        <td><textarea name="wpspf_servicetype" style="width:100%;" required="required" placeholder="seperate service type by | e.g. type one | type two | type three"><?php echo get_option( 'wpspf_servicetype' ); ?></textarea></td>
        </tr>
		
        <tr valign="top">
        <th scope="row"><?php echo __( 'Enable/Disable', 'wpspf_with_authorize.net' ); ?></th>
        <td><input type="checkbox" name="wpspfnet_enable" value="1" <?php if ( trim(get_option( 'wpspfnet_enable' ))==1 ){ echo 'checked'; } ?> /><?php echo __( 'Enable Authorize.Net', 'wpspf_with_authorize.net' ); ?></td>
        </tr>
         
        <tr valign="top">
        <th scope="row"><?php echo __( 'API Login ID', 'wpspf_with_authorize.net' ); ?></th>
        <td><input type="text" name="wpspf_apiloginid" value="<?php echo esc_attr( get_option('wpspf_apiloginid') ); ?>" required="required" /></td>
        </tr>
        
        <tr valign="top">
        <th scope="row"><?php echo __( 'Transaction Key', 'wpspf_with_authorize.net' ); ?></th>
		...
		<tr valign="top">
        <th scope="row"><?php echo __( 'Transaction Mode', 'wpspf_with_authorize.net' ); ?></th>
        <td><input type="checkbox" name="wpspf_transactionmode" value="1" <?php if ( trim(get_option( 'wpspf_transactionmode' ))==1 ){ echo 'checked'; } ?> /><?php echo __( 'Enable Authorize.Net sandbox (Live Mode if Unchecked)', 'wpspf_with_authorize.net' ); ?></td>
        </tr>	
...
function wpspf_paymentform(){
	...
	if(isset($_POST['wpspf_payment'])){
			// Testing, is it a real transaction
			$environment = ( trim(get_option( 'wpspf_transactionmode' ))==1 ) ? 'TRUE' : 'FALSE';	
			if(trim($_POST['service_phone'])!='' ){ 
				$service_phone=trim($_POST['service_phone']);
			}else{ 
				$service_phone='';
			}
			if(trim($_POST['service_email'])!='' ){ 
			...
			$payload = array(
				...
				// Order total
				"x_amount"             	=> trim($_POST['payment_amount']),

				// Credit Card Information
				"x_card_num"           	=> str_replace( array(' ', '-' ), '', $_POST['wpspf_authorizenet_card-number'] ),
				"x_card_code"          	=> ( isset( $_POST['wpspf_authorizenet_card-cvc'] ) ) ? $_POST['wpspf_authorizenet_card-cvc'] : '',
				"x_exp_date"           	=> str_replace( array( '/', ' '), '', $_POST['wpspf_authorizenet_card-expiry'] ),

				"x_type"               	=> 'AUTH_CAPTURE',
				"x_invoice_num"        	=> str_replace( "#", "", trim($_POST['invoice_number'])),
				...
				// Billing Information
				"x_first_name"         	=> trim($_POST['customer_first_name']),
				"x_last_name"          	=> trim($_POST['customer_last_name']),
				"x_address"            	=> (isset($_POST['billing_address'])) ? $_POST['billing_address'] : '',
				"x_city"              	=> trim($_POST['billing_city']),
				"x_state"              	=> trim($_POST['billing_state']),
				"x_zip"                	=> trim($_POST['billing_zipcode']),
				"x_country"            	=> trim($_POST['billing_country']),
				"x_phone"              	=> $service_phone,
				"x_email"              	=> $service_email,

				// Shipping Information
				"x_ship_to_first_name" 	=> trim($_POST['customer_first_name']),
				"x_ship_to_last_name"  	=> trim($_POST['customer_last_name']),
				"x_ship_to_company"    	=> '',
				"x_ship_to_address"    	=> (isset($_POST['service_address'])) ? $_POST['service_address'] : '',
				"x_ship_to_city"       	=> trim($_POST['service_city']),
				"x_ship_to_country"    	=> trim($_POST['service_country']),
				"x_ship_to_state"      	=> trim($_POST['service_state']),
				"x_ship_to_zip"        	=> trim($_POST['service_zip']),
			...
			if ( is_wp_error( $response ) ) 
				throw new Exception( __( 'We are currently experiencing problems trying to connect to this payment gateway. Sorry for the inconvenience.', 'wpspf_with_authorize.net' ) );

			if ( empty( $response['body'] ) )
				throw new Exception( __( 'Authorize.net\'s Response was empty.', 'wpspf_with_authorize.net' ) );
		...
		<table id="wc-authorizenet_lightweight-cc-form" class="wc-credit-card-form wc-payment-form">
			<tr>
				<th><?php echo __( 'First Name', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
				<th> <?php echo __( 'Last Name', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
				<th><?php echo __( 'Payment Amount ', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
				<th> <?php echo __( 'Invoice Number(s) (Optional)', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="invoice_number" id="invoice_number" placeholder="Invoice Number(s)" class="form-field"></td>
			</tr>
			<?php if( trim(get_option( 'wpspfnet_enable_servicetype' ))==1 ){ 
			...
			?>
			<tr>
				<th>  <?php echo __( 'Service type', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
				<th><?php echo __( 'Phone (Optional)', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="service_phone" id="service_phone" placeholder="Phone" class="form-field"></td>
			</tr>
			<tr>
				<th><?php echo __( 'Email (Optional)', 'wpspf_with_authorize.net' ); ?><br/><span style="font-size:10px;">For receiving payment confirmation<?php echo __( 'Last Name', 'wpspf_with_authorize.net' ); ?></span></th>
				<td><input type="email" name="service_email" id="service_email" placeholder="Email" class="form-field"></td>
			</tr>
			
			
			<tr>
				<th><?php echo __( 'Billing Address (if different from Service Address)', 'wpspf_with_authorize.net' ); ?></th>
				<td></td>
			</tr>
			<tr>
				<th><?php echo __( 'Address', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="billing_address" id="billing_address" placeholder="Address" class="form-field"></td>
			</tr>
			<tr>
				<th><?php echo __( 'City', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="billing_city" id="billing_city" placeholder="City" class="form-field"></td>
			</tr>
			<tr>
				<th><?php echo __( 'State', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="billing_state" id="billing_state" placeholder="State" class="form-field"></td>
			</tr>
			<tr>
				<th><?php echo __( 'Zip Code', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="billing_zipcode" id="billing_zipcode" placeholder="Zip Code" class="form-field"></td>
			</tr>
			<tr>
				<th><?php echo __( 'Country', 'wpspf_with_authorize.net' ); ?></th>
				<td><input type="text" name="billing_country" id="billing_country" placeholder="Country Name" class="form-field"></td>
			</tr>
			
			<tr>
				<th>  <?php echo __( 'Payment Method', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
				<th> <?php echo __( 'Credit Card Number', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
				<th> <?php echo __( 'Expiration Date (MM/YY)', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>
			<th> <?php echo __( 'Security Code', 'wpspf_with_authorize.net' ); ?><span class="required">*</span></th>
			...
			<tr>	<td></td>		
			<td><input type="submit" name="wpspf_payment" class="btn button form-field" value="Pay Your Bill"></td></tr>
			</table>
	    </form>
<script>
